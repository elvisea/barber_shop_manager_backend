 ...
 * 
 * USO NO SISTEMA:
 * - Chamado pelo MessageBufferService
...
export class AIToolExecutorService {
  ...
  constructor(private readonly toolRegistry: ToolRegistryService) {
    ...
    this.logger.log(`⚙️ [EXECUTOR] Tools disponíveis: ${this.toolRegistry.listAvailableTools().join(', ')}`);
  }
  ...
  async executeToolCall(
    ...
    this.logger.log(`🛠️ [EXECUTOR] Iniciando execução da tool: "${toolCall.name}"`);
    this.logger.log(`🛠️ [EXECUTOR] Argumentos recebidos:`, JSON.stringify(toolCall.arguments, null, 2));
    ...
    try {
      ...
      this.logger.log(`✅ [EXECUTOR] Tool "${toolCall.name}" executada com sucesso`);
      this.logger.log(`✅ [EXECUTOR] Resultado:`, JSON.stringify(result, null, 2));
      ...
      this.logger.error(`❌ [EXECUTOR] Erro ao executar tool "${toolCall.name}":`, error);
  ...
  async executeToolCalls(
    ...
    this.logger.log(`🔄 [EXECUTOR] Iniciando execução de ${toolCalls.length} tool calls`);
    this.logger.log(`🔄 [EXECUTOR] Tools a executar: ${toolCalls.map(tc => tc.name).join(', ')}`);
    ...
    for (const toolCall of toolCalls) {
      this.logger.log(`🔄 [EXECUTOR] Executando tool ${results.length + 1}/${toolCalls.length}: "${toolCall.name}"`);
    ...
    this.logger.log(`📊 [EXECUTOR] Execução concluída: ${successCount} sucessos, ${errorCount} erros`);
    this.logger.log(`📊 [EXECUTOR] Resultados finais:`, JSON.stringify(results, null, 2));
  ...
  generateToolResultsContext(toolResults: ToolExecutionResult[]): any[] {
    this.logger.log(`📊 [CONTEXTO] Gerando contexto para ${toolResults.length} resultados de tools`);

    const contextMessages: any[] = [];

    for (const result of toolResults) {
      if (result.success) {
        this.logger.log(`✅ [CONTEXTO] Tool "${result.toolCall.name}" executada com sucesso`);
        ...
        this.logger.error(`❌ [CONTEXTO] Tool "${result.toolCall.name}" falhou: ${result.error}`);
    ...
    this.logger.log(`📊 [CONTEXTO] Contexto gerado com ${contextMessages.length} mensagens`);
    this.logger.log(`📊 [CONTEXTO] Mensagens:`, JSON.stringify(contextMessages, null, 2));
  ...
  validateToolCall(toolCall: ToolCall): boolean {
    ...
    if (!toolCall.name || !toolCall.arguments) {
      this.logger.warn(`⚠️ [VALIDAÇÃO] Tool call inválida: nome ou argumentos ausentes`);
      return false;
    }
    ...
    if (toolExists) {
      this.logger.log(`✅ [VALIDAÇÃO] Tool "${toolCall.name}" validada com sucesso`);
    } else {
      this.logger.warn(`⚠️ [VALIDAÇÃO] Tool "${toolCall.name}" não encontrada no registry`);
    }
  ...
  getStats(): { totalTools: number; toolNames: string[] } {
    ...
    this.logger.log(`📈 [STATS] Estatísticas do executor:`, JSON.stringify(stats, null, 2));
