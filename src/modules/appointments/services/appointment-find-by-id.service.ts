...
export class AppointmentFindByIdService {
  private readonly logger = new Logger(AppointmentFindByIdService.name);
  ...
  async execute(
    ...
    this.logger.log(
      `Finding appointment ${appointmentId} in establishment ${establishmentId}`,
    );
    ...
    this.logger.log(`Appointment ${appointmentId} found and access allowed`);
