...
import { MemberVerifyEmailResponseDto } from '../../members/dtos/member-verify-email-response.dto';

import { SwaggerErrors } from '@/common/swagger-errors';
import { ErrorCode } from '@/enums/error-code';

/**
 * Documentação completa do endpoint de verificação de email de membro de estabelecimento
 *
 * Este decorator composto aplica toda a documentação Swagger necessária
 * para o endpoint POST /members/email-verification/verify
 */
export function VerifyUserEstablishmentEmailDocs() {
  return applyDecorators(
    ApiOperation({
      summary: 'Verify establishment member email',
      description:
